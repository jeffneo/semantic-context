"""Parse-health report and review sample for a parse_log.py run.

PARSE_HEALTH.md answers "can the graph trust these records?":
- resolution status, split by whether the job succeeded in BigQuery (a query that
  failed there should fail here too);
- an independent check: BigQuery's own referenced_tables (base tables, views
...
def short(t: str | None) -> str:
    return ".".join(t.split(".")[1:]) if t and t.count(".") >= 2 else (t or "?")
...
def crosscheck(shapes, groups, cat) -> tuple[Counter, list]:
    """Parser tables (views expanded) vs BigQuery referenced_tables, per shape."""
    view_src = {}
    for s in shapes:
        if s["origin"] == "catalog_view":
            for w in s["record"].get("writes", []):
                view_src[w["table"]] = {t["id"] for t in s["record"].get("tables", []) if t["id"] != w["table"]}
    ...
    fam = set(cat.get("shard_families", []))
    rules = [(re.compile(r["pattern"]), r["replace"]) for r in cat.get("rules", [])]

    def canon(p, d, n):
        home = aliases.get(f"{p}.{d}")
        if home:
            p, d = home.split(".", 1)
        for rx, rep in rules:
            n = rx.sub(rep, n)
        head, _, tail = n.rpartition("_")
        if head and f"{p}.{d}.{head}_" in fam and (tail.isdigit() or tail.endswith("*")):
            n = head + "_*"
        return f"{p}.{d}.{n}"
    ...
    for g in groups:
        if g["jobs"] > g["cache_hits"] and not g["errors"] and g["referenced_tables"]:
            ref.setdefault(g["text_id"], {canon(x["project_id"], x["dataset_id"], x["table_id"])
...
def write(shapes, groups, fps, cat, timing, work: Path):
    ...
    L = ["# Parse health", ""]
    w = timing["workers"]
    L += [f"Parser `{w['parser']}` ({'compiled' if w.get('compiled') else 'pure Python'}), catalog "
          f"`{cat['version']}`" + (f", **sample {timing['sample']:.0%} of distinct texts**" if timing["sample"] else ""),
          ""]
    L += ["## Volume", "", "| | count |", "|---|---|",
          f"| Jobs | {timing['jobs']:,} |",
          f"| Groups from T0 (text × principal × week, aggregated in BigQuery) | {timing['groups']:,} |",
          f"| Distinct texts (T1 fingerprinted) | {timing['texts']:,} |",
          f"| Shapes from the log (T2 resolved) | {len(log):,} |",
          f"| View definitions from the catalog (T2 resolved) | {len(views):,} |",
          f"| Jobs per shape | {timing['jobs'] / max(len(log), 1):,.0f} |", ""]
    t1 = timing["t1_server_ms"] / max(timing["t1_items"], 1)
    t2 = timing["t2_server_ms"] / max(timing["t2_items"], 1)
    L += ["## Throughput", "",
          f"- T1 fingerprint: {timing['t1_items']:,} texts, {timing['t1_wall_s']:.1f}s wall, "
          f"**{t1:.2f} ms/text** of worker time",
          f"- T2 resolve: {timing['t2_items']:,} shapes, {timing['t2_wall_s']:.1f}s wall, "
          f"**{t2:.2f} ms/shape** of worker time", ""]

    L += ["## Resolution status", "",
          "| | ok | partial | error |", "|---|---|---|---|"]
    for name, grp in (("Ran successfully in BigQuery", okq), ("Failed in BigQuery", bad),
                      ("Catalog view definitions", views)):
        c = Counter(s["record"]["status"] for s in grp)
        L.append(f"| {name} ({len(grp):,}) | {c['ok']:,} | {c['partial']:,} | {c['error']:,} |")
    L.append("")
    L += ["A shape that failed in BigQuery is expected to be partial or error here (typos, "
    ...
    def misses(grp):
        ...
        names = Counter((u["kind"], u["reason"], u["name"]) for s in grp for u in s["record"].get("unresolved", []))
        return by, names
    ...
    L.append("")
    by, _ = misses(bad)
    L += ["### Unresolved references in shapes that failed in BigQuery", "",
    ...
    L += ["## Cross-check against BigQuery's referenced_tables", "",
          f"For every successful shape whose jobs were not all cache hits: the base tables BigQuery "
    ...
    for d in diffs[:10]:
        L += ["", f"- {d[0]}: parser only {d[1]}, BigQuery only {d[2]}", f"  `{d[3]}`"]
    L.append("")
    ...
    for s in okq + views:
        r = s["record"]
        tables |= {t["id"] for t in r.get("tables", []) if t["kind"] not in ("system", "transient", "unknown")}
        ...
        for wr in r.get("writes", []):
            for c in wr["columns"]:
                lin[c["kind"]] += 1
        for t, v in (r.get("scan", {}).get("partition_filter") or {}).items():
            pf[v] += 1
    ...
    L += ["## Extracted (successful shapes + view definitions)", "",
    ...
    L += [f"Catalog tables never referenced by any successful shape: {len(unseen)}"
...
STRATA = [  # (label, n, predicate on (shape, principal, sql))
    ("Human ad-hoc SELECT with a join", 3,
     lambda s, p, q: p == "human" and s["record"].get("joins") and s["record"].get("statement_type") == "SELECT"),
    ("Human GA4 query (wildcard table, UNNEST)", 1, lambda s, p, q: p == "human" and "events_*" in q),
    ("Human SELECT * ... LIMIT", 1,
     lambda s, p, q: p == "human" and s["record"].get("scan", {}).get("select_star") and s["record"]["scan"].get("limit")),
    ("Human query that failed in BigQuery", 1, lambda s, p, q: p == "human" and not ran_ok(s)),
    ("Human sandbox CTAS", 1, lambda s, p, q: p == "human" and s["record"].get("statement_type") == "CREATE_TABLE_AS_SELECT"),
    ("Looker explore query", 2, lambda s, p, q: p == "looker" and s["record"].get("statement_type") == "SELECT"
     and "Looker Query Context" in q),
    ("Looker PDT build", 1, lambda s, p, q: p == "looker" and s["record"].get("statement_type") == "CREATE_TABLE_AS_SELECT"),
    ("dbt incremental MERGE", 2, lambda s, p, q: p == "dbt" and s["record"].get("statement_type") == "MERGE"),
    ("dbt view (staging or union)", 2, lambda s, p, q: p == "dbt" and s["record"].get("statement_type") == "CREATE_VIEW"),
    ("dbt table (CTAS)", 1, lambda s, p, q: p == "dbt" and s["record"].get("statement_type") == "CREATE_TABLE_AS_SELECT"),
    ("dbt test or freshness check", 2, lambda s, p, q: p == "dbt" and s["record"].get("statement_type") == "SELECT"),
...
def principal_class(s) -> str:
    """For choosing review examples only - the pipeline's actor model is Stage 3."""
    top = next(iter((s.get("stats") or {}).get("principals", {})), "") or ""
    for key, cls in (("dbt", "dbt"), ("looker", "looker"), ("fivetran", "fivetran"), ("elementary", "monitor"),
                     ("composer", "legacy"), ("hightouch", "reverse_etl"), ("tableau", "tableau")):
        if key in top:
            return cls
...
def review(shapes, fps, work: Path):
    log = sorted((s for s in shapes if s["origin"] == "log"),
    ...
    for i, (label, s) in enumerate(picked, 1):
        ...
        L += ["```sql"] + sql[:45] + (["-- ... (%d more lines)" % (len(sql) - 45)] if len(sql) > 45 else []) + ["```", ""]
        tabs = ", ".join(f"`{short(t['id'])}` ({t['kind']})" for t in r.get("tables", []))
        L.append(f"- **Tables:** {tabs or '-'}")
        for j in r.get("joins", []):
            a, b = j["left"], j["right"]
            fmt = lambda x: f"`{short(x['table'])}.{x['column']}{'.' + x['path'] if x['path'] else ''}`" + \
        ...
        for wr in r.get("writes", []):
            ...
            for c in wr["columns"][:12]:
                src = ", ".join(f"{short(f['table'])}.{f['column']}{'.' + f['path'] if f.get('path') else ''}"
                                for f in c["from"]) or "-"
            ...
            for k in wr.get("merge_keys", []):
                L.append(f"    - merge key `{k['target']}` ← {', '.join(short(f['table']) + '.' + f['column'] for f in k['from'])}")
        ...
        if sc.get("partition_filter") or sc.get("select_star"):
            L.append(f"- **Scan:** partition filter {sc.get('partition_filter')}; select * {sc.get('select_star')}; "
                     f"limit {sc.get('limit')}")
        reads = r.get("reads", [])
        rs = "; ".join(f"{short(x['table']).split('.')[-1]}.{x['column']}{'.' + x['path'] if x['path'] else ''}"
                       f" [{','.join(x['roles'] + [('~' + d) for d in x['derived_roles']])}"
                       f"{' ' + '/'.join(x['agg']) if x['agg'] else ''}]" for x in reads[:14])
        L.append(f"- **Reads** ({len(reads)}): {rs}{' ...' if len(reads) > 14 else ''}")
        if r.get("unresolved"):
            L.append(f"- **Unresolved:** " + "; ".join(f"{u['kind']} `{u['name']}` ({u['reason']})"
