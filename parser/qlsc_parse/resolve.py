...
def statement_type(t: exp.Expression) -> str:
    ...
    return {exp.Insert: "INSERT", exp.Merge: "MERGE", exp.Delete: "DELETE", exp.Update: "UPDATE"}.get(
        type(t), type(t).__name__.upper()
    )
...
def trace_expr(ctx: Ctx, sc: Scope, e: exp.Expression) -> list[Origin]:
    ...
    cols = [
        ...
        for c in cols
        if not any(
            isinstance(a, (exp.Subquery, exp.Select)) and a is not body for a in _ancestors_until(c, body)
        )
    ]
            ...
            for o in trace_expr(ctx, src, arr):
                path = (
                    ".".join(
                        ...
                        for p in (
                            o.path,
                            col.name if col.name.lower() != (e.alias or "").lower() else None,
                            field_path(col),
                        )
                        if p
                    )
                ...
                out.append(
                    Origin(o.table, o.column, "unnest" if KIND_RANK[o.kind] < 3 else o.kind, o.fns, path)
                )
        return out
    if isinstance(e, exp.SetOperation):
        ...
        star = (
            next(
                (
                    ...
                    if isinstance(x, exp.Star) or (isinstance(x, exp.Column) and isinstance(x.this, exp.Star))
                ),
                None,
            )
...
def add_read(ctx: Ctx, o: Origin, role: str, agg: str | None):
    r = ctx.reads.setdefault(
        (o.table, o.column, o.path), {"roles": set(), "derived_roles": set(), "agg": set()}
    )
        ...
        ctx.filters.append(
            {
                ...
                "path": o.path,
                "op": op,
                "wrap": list(o.fns[::-1]) + wraps,
                "slots": slots,
                "clause": clause,
                "scope": scope_label(sc),
                "via": o.kind,
                "_scope": id(sc),
            }
        )
...
def analyze_scope(ctx: Ctx, sc: Scope):
    ...
    for name, src in sc.sources.items():
        ...
        pushed = any(
            fl["_scope"] == id(sc)
            and fl["table"] == f
            and fl["column"].lower() == pcol.lower()
            and fl["clause"] == "where"
            for fl in ctx.filters
        )
        if pcol == "_TABLE_SUFFIX":
            pushed = pushed or any(
                fl["_scope"] == id(sc) and fl["column"].lower() == "_table_suffix" for fl in ctx.filters
            )
...
def outputs(ctx: Ctx, root_sc: Scope) -> list[tuple[str, list[Origin]]]:
    ...
    if isinstance(e, exp.SetOperation):
        ...
        return [
            (n, positional(ctx, root_sc, i), first.expression.expressions[i]) for i, n in enumerate(names)
        ]
    if isinstance(e, exp.Select):
...
def lineage_entry(ctx: Ctx, target: str, name: str, origins: list[Origin], expr=None) -> dict:
    ...
    return {
        "target": spelled,
        "from": [
            {"table": o.table, "column": o.column, "path": o.path, **({"control": True} if o.control else {})}
            for o in origins
        ],
        "kind": kind,
        "fn": fns,
...
def writes_of(ctx: Ctx, t: exp.Expression, root_scopes: dict) -> list[dict]:
    ...
    if st == "MERGE":
        ...
        for when in t.args.get("whens").expressions if t.args.get("whens") else []:
            ...
            if (
                isinstance(then, exp.Insert)
                and isinstance(then.this, exp.Var)
                and then.this.name.upper() == "ROW"
            ):
        ...
        for eq in on.find_all(exp.EQ) if on is not None else []:
            ...
            if isinstance(a, exp.Column) and isinstance(b, exp.Column):
                ...
                keys.append(
                    {
                        "target": ctx.catalog.column(target, tc.name) or tc.name,
...
def resolve(
    sql: str, catalog: Catalog, default_project: str | None = None, shape_id: str | None = None
) -> dict:
    ...
    for t in trees:
        ...
        if isinstance(body, exp.Select) or isinstance(t, exp.Select):
            ...
            ctx.scan["select_star"] |= any(
                isinstance(x, exp.Star) or (isinstance(x, exp.Column) and isinstance(x.this, exp.Star))
    ...
    status = (
        "error" if errors and not ctx.reads and not writes else ("partial" if errors or real_misses else "ok")
    )
    rec.update(
        {
            "status": status,
            ...
            "reads": [
                {
                    ...
                    "derived_roles": sorted(v["derived_roles"] - v["roles"]),
                    "agg": sorted(v["agg"]),
                }
                for (t, c, p), v in sorted(
                    ctx.reads.items(), key=lambda kv: (kv[0][0], kv[0][1], kv[0][2] or "")
                )
            ],
            ...
            "scan": ctx.scan,
            "unresolved": [
                {"kind": k, "name": n, "reason": r} for (k, n), r in sorted(ctx.unresolved.items())
            ],
        }
    )
    if errors:
