...
RULES = [
    {"pattern": r"_[0-9a-f]{12,}$", "replace": "_{hex}"},
...
CAT = Catalog(
    {
        ...
        "tables": {
            "p.raw.customer": {
                "kind": "TABLE",
                "partition": None,
                "columns": {"CIF_NO": "STRING", "NAME": "STRING", "SSN": "STRING", "OPEN_DT": "DATE"},
            },
            "p.raw.account": {
                "kind": "TABLE",
                "partition": "BAL_DT",
                "columns": {"ACCT_ID": "INT64", "CIF_NO": "STRING", "BAL": "NUMERIC", "BAL_DT": "DATE"},
            },
            "p.dw.dim_customer": {
                "kind": "TABLE",
                "partition": None,
            ...
            "p.dw.dim_branch": {
                "kind": "TABLE",
                "partition": None,
                "columns": {"branch_id": "STRING", "branch_name": "STRING", "parent_branch_id": "STRING"},
            },
            "p.dw.stg_customer": {
                "kind": "VIEW",
                "partition": None,
                "columns": {"cif_number": "STRING", "name": "STRING"},
            ...
            "p.ga.events_*": {
                ...
                "columns": {
                    ...
                    "collected_traffic_source": "STRUCT<manual_campaign_name STRING>",
...
def read(rec, table, column, path=None):
    return next(
        r for r in rec["reads"] if r["table"].endswith(table) and r["column"] == column and r["path"] == path
    )


def lineage(rec):
    return {
        c["target"]: (c["kind"], sorted(f["table"].split(".")[-1] + "." + f["column"] for f in c["from"]))
        for w in rec["writes"]
        for c in w["columns"]
    }
...
def test_aliased_join():
    r = R(
        "SELECT c.segment, SUM(t.amount_cents) FROM `p.dw.fct_txn` t "
...
def test_join_through_cte_rename_traces_to_base():
    r = R(
        "WITH a AS (SELECT x.CIF_NO AS cid FROM `p.raw.customer` x) "
...
def test_shadowed_alias_in_cte_and_outer_query():
    ...
    r = R(
        "WITH k AS (SELECT a.cif_number FROM `p.dw.dim_customer` a) "
...
def test_join_wrappers_recorded():
    r = R(
        "SELECT 1 FROM `p.raw.account` a JOIN `p.dw.dim_customer` d "
...
def test_join_on_derived_transform_is_marked():
    r = R(
        "SELECT 1 FROM (SELECT CONCAT('C', CIF_NO) AS k FROM `p.raw.customer`) s "
...
def test_aggregate_order_by_column_is_not_a_join_side():
    ...
    r = R(
        "WITH f AS (SELECT ARRAY_AGG(branch_id ORDER BY parent_branch_id)[OFFSET(0)] AS first_seg "
...
def test_control_origin_marked_in_lineage():
    r = R(
        "CREATE TABLE `p.dw.t5` AS SELECT ARRAY_AGG(branch_id ORDER BY parent_branch_id)[OFFSET(0)] AS first_b "
...
def test_case_condition_is_not_a_join_side():
    r = R(
        "SELECT 1 FROM (SELECT CASE WHEN segment = 'A' THEN cif_number END AS k FROM `p.dw.dim_customer`) s "
...
def test_in_subquery_is_a_semi_join():
    r = R(
        "SELECT txn_id FROM `p.dw.fct_txn` WHERE customer_key IN "
...
def test_correlated_exists_is_a_join():
    r = R(
        "SELECT d.cif_number FROM `p.dw.dim_customer` d WHERE EXISTS "
...
def test_self_join_on_hierarchy():
    r = R(
        "SELECT b.branch_name, p.branch_name FROM `p.dw.dim_branch` b "
...
def test_on_clause_literal_is_a_filter_not_a_join():
    r = R(
        "SELECT 1 FROM `p.dw.fct_txn` t JOIN `p.dw.dim_customer` c "
...
def test_unnest_event_params_filter_and_path():
    sql = (
        "SELECT (SELECT value.int_value FROM UNNEST(event_params) WHERE key = 'ga_session_id') AS s "
...
def test_partition_filter_after_aggregation_is_not_pushed():
    r = R(
        "SELECT * FROM (SELECT post_date, SUM(amount_cents) AS s FROM `p.dw.fct_txn` GROUP BY 1) "
...
def test_qualify_and_window_roles():
    r = R(
        "SELECT customer_key, txn_id FROM `p.dw.fct_txn` "
...
def test_struct_field_path():
    r = R(
        "SELECT collected_traffic_source.manual_campaign_name FROM `p.ga.events_*` "
...
def test_ctas_lineage_kinds():
    r = R(
        "CREATE OR REPLACE TABLE `p.dw.cust_summary` AS SELECT c.cif_number, c.segment AS seg, "
...
def test_view_definition_lineage():
    r = R(
        CAT.get("p.dw.stg_customer")["view_sql"].replace(
            "SELECT", "CREATE VIEW `p.dw.stg_customer` AS SELECT", 1
        )
    )
    assert r["writes"][0]["mode"] == "VIEW"
    assert lineage(r) == {
        "cif_number": ("rename", ["customer.CIF_NO"]),
        "name": ("passthrough", ["customer.NAME"]),
    }
...
def test_insert_positional_lineage():
    r = R(
        "INSERT INTO `p.dw.dim_branch` (branch_id, branch_name) SELECT CIF_NO, UPPER(NAME) FROM `p.raw.customer`"
    )
    L = lineage(r)
...
def test_dbt_merge_lineage_and_keys():
    ...
    assert w["merge_keys"] == [
        {"target": "cif_number", "from": [{"table": "p.raw.customer", "column": "CIF_NO"}]}
    ]


def test_union_all_lineage_is_positional():
    r = R(
        "CREATE VIEW `p.dw.v` AS WITH a AS (SELECT CIF_NO AS id, 'CORE' AS src FROM `p.raw.customer`), "
...
def test_union_of_star_branches_over_unaliased_ctes():
    ...
    r = R(
        "CREATE TABLE `p.dw.u` AS WITH a AS (SELECT CIF_NO AS id, NAME AS nm, CAST(NULL AS STRING) AS x "
...
def test_ctas_with_parenthesized_body():
    r = R(
        "CREATE OR REPLACE TABLE `p.dw.t2` PARTITION BY post_date AS (SELECT post_date, txn_id FROM `p.dw.fct_txn`)"
    )
    assert lineage(r) == {
        "post_date": ("passthrough", ["fct_txn.post_date"]),
...
def test_count_star_is_an_aggregate_not_a_constant():
    r = R(
        "CREATE TABLE `p.dw.t3` AS SELECT segment, COUNT(*) AS n, 'X' AS k FROM `p.dw.dim_customer` GROUP BY 1"
    )
...
def test_looker_pdt_generations_share_one_table():
    ...
    assert r["status"] == "ok" and r["tables"] == [
        {"id": "p.looker_scratch.LR_{id}_customer_facts", "kind": "table"}
    ]


def test_scaling_constant_in_lineage():
    r = R(
        "CREATE TABLE `p.dw.t4` AS SELECT SUM(amount_cents) / 100 AS spend_usd, SUM(amount_cents) AS raw_sum "
        "FROM `p.dw.fct_txn`"
    )
...
def test_transient_staging_table_is_expected():
    r = R(
        "MERGE `p.raw.customer` T USING `p.raw.stage_customer_25ad1233b062` S ON T.CIF_NO = S.CIF_NO "
