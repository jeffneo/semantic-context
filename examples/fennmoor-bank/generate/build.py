...
Reads spec/warehouse/*.yaml + spec/concepts.yaml and writes build/:
...
Usage:  uv run examples/fennmoor-bank/generate/build.py [--strict]
...
def load():
    ...
    concepts_doc = yaml.safe_load((SPEC / "concepts.yaml").read_text())
    tables: list[Table] = []
...
PHRASES = {
    ...
    ("CIF",): "cif_number",
    ("OPT", "IN"): "opted_in",
    ("GL", "ACCT"): "gl_account_code",
    ("CRT", "TS"): "created_at",
    ("LST", "UPD", "TS"): "last_updated_at",
    ("CHG", "TS"): "changed_at",
...
def expand_upper(name: str) -> str:
    ...
    while i < len(toks):
        for n in (3, 2, 1):
            ...
            out.append(TOKENS.get(toks[i], toks[i].lower()))
            i += 1
    return "_".join(out)
    ...
    for c in tree.find_all(exp.Column):
        ...
        if len(parts) >= 2 and parts[0] in aliases:
            ...
            if kind == "table":
                ...
                if not hit:
                    err(
                        f"{where}: {parts[0]}.{parts[1]} is not an output of CTE/subquery {aliases[parts[0]][2]}"
                    )
                else:
            ...
            and aliases[parts[0]][0] == "unnest"
            or in_subquery
            or len(parts) == 1
            and parts[0].lower() in PSEUDO_COLUMNS | {"current_date", "current_timestamp"}
        ):
            continue
        else:
...
def is_passthrough(e) -> bool:
    ...
    if isinstance(node, exp.Column):
        return True
    return isinstance(node, (exp.AnyValue, exp.Min, exp.Max)) and isinstance(node.this, exp.Column)
...
def resolve_query(q, fqn_to_table: dict, ctes: dict) -> list[tuple[str, list[str], bool]]:
    ...
    if isinstance(q, exp.SetOperation) if hasattr(exp, "SetOperation") else isinstance(q, exp.Union):
        ...
        return [
            (
                n,
                lv + (right[i][1] if i < len(right) else []),
                pt and (right[i][2] if i < len(right) else True),
            )
...
def resolve_copy(t: Table, idx: Index) -> bool:
    ...
    for c in src.columns:
        ...
        cols.append(Column(new_name, c.type, f"s.{c.name}", c.concept, flags, [f"{src.fqn}.{c.name}"]))
        if c.name in rename:
...
def valid_type(t: str) -> bool:
    ...
    if t.startswith("STRUCT<") and t.endswith(">"):
        ...
        fields.append(cur)
        return all(
            len(f.strip().split(None, 1)) == 2 and valid_type(f.strip().split(None, 1)[1]) for f in fields
        )
    return False
...
def write_answer_keys(findings, questions, tables, idx, joins):
    ...
    for fid, f in findings.items():
        out["findings"][fid] = {
            **f,
            "objects": [fq(o) for o in f.get("objects", [])],
            "tagged_tables": sorted(
                t.fqn for t in tables if fid in (t.spec.get("planted"), t.spec.get("trap"))
            ),
        }
    for qid, q in questions.items():
        out["questions"][qid] = {
        ...
        if t.kind == "base":
            ...
            for m in t.spec.get("mixins", []):
                if m not in mixins:
                    ...
                    continue
                t.columns += [
                    parse_column(c, f"mixin {m}")
                    for c in mixins[m]["columns"]
        ...
        if t.kind in ("staging", "model"):
            for (lt, lc), (rt, rc) in build_model_sql(t, idx, fqn_to_table):
                joins.append(
                    {
                        "model": t.fqn,
                        "writer": t.spec.get("writer"),
                        "left": f"{lt.fqn}.{lc.name}",
                        "right": f"{rt.fqn}.{rc.name}",
                        "left_concept": lc.concept,
                        "right_concept": rc.concept,
                    }
                )
    ...
    for j in joins:
        ...
        if lc.get("id_space") and rc.get("id_space") and lc["id_space"] != rc["id_space"]:
            ...
            if j["writer"] in production or not model.spec.get("trap"):
                err(
                    f"{j['model']}: joins {j['left']} ({j['left_concept']}) to "
...
def emit(estate, concepts_doc, tables: list[Table], joins: list[dict]):
    ...
    order = sorted(
        tables,
        key=lambda t: (
            layer_order.index(t.layer) if t.layer in layer_order else 99,
            t.project,
            t.dataset,
            t.name.lower(),
        ),
    ...
    wh = {
        "org": estate["org"],
        "log_window": estate["log_window"],
        "projects": estate["projects"],
        "datasets": estate["datasets"],
        "teams": estate["teams"],
        "service_accounts": estate["service_accounts"],
        "subjects": estate["subjects"],
        "tables": [],
        "concepts": concepts,
        "glossary": concepts_doc.get("glossary", {}),
    }
...
def write_summary(estate, concepts, tables: list[Table], joins):
    ...
    L += [
        ...
        "",
        f"Generated by `examples/fennmoor-bank/generate/build.py`. {len(tables)} logical tables, {phys} physical "
    ...
    for t in tables:
        by[(t.project, t.layer)][0].add(t.dataset)
        by[(t.project, t.layer)][1] += 1
    for (p, layer), (dss, n) in sorted(by.items()):
        L.append(f"| {p} | {layer} | {len(dss)} | {n} |")
    L += ["", "## Status", ""]
