...
The pipeline reads the export like any other input (qlsc align); it never reads spec/.
Deterministic (seeded). Usage: uv run examples/fennmoor-bank/generate/make_catalog.py
"""
...
OUT = EXAMPLE / "designed" / "catalog.json"
CURATED = {"raw", "staging", "mart", "intermediate", "ml"}
ORPHANS = [
    (
        "safe_deposit_box",
        "Safe Deposit Box",
        "A box rented to a customer in a branch vault, identified by box number and branch.",
    ),
    (
        "wire_transfer",
        "Wire Transfer",
        "A same-day funds transfer to or from another bank over Fedwire or SWIFT.",
    ),
    (
        "mortgage_escrow",
        "Mortgage Escrow Balance",
        "Funds held with a mortgage to pay property tax and insurance.",
    ),
    ...
    (
        ...
        "Whether a checking account is enrolled to cover overdrafts from a linked account.",
    ),
    (
        "insurance_product",
        "Credit Insurance Product",
        "Credit life or disability insurance sold with a loan or card.",
    ),
]
...
def main() -> int:
    ...
    terms = {
        c: {
            "id": c,
            "name": term_name(c, concepts[c].get("desc", c)),
            "definition": concepts[c].get("desc", ""),
            "status": "approved",
            "columns": [],
        }
        for c in kept
        ...
        assets.append(
            {
                ...
                "certified": bool(stale or rnd.random() < 0.4),
                "columns": cols,
            }
        )
    ...
    OUT.write_text(
        json.dumps(
            {
                "source": "Fennmoor Data Catalog - synthetic export (generated from the spec with "
                "deliberate gaps, stale entries and three wrong bindings)",
                "glossary": sorted(terms.values(), key=lambda x: x["id"]),
                "assets": assets,
            },
            indent=1,
        )
    ...
    print(
        f"{len(terms)} glossary terms ({len(ORPHANS)} orphans), {bound} column bindings ({len(WRONG)} wrong), "
