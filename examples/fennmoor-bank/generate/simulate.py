...
Inputs:  build/warehouse.json, build/templates.json, spec/simulation.yaml
...
Usage: uv run examples/fennmoor-bank/generate/simulate.py [--days N] [--scale X] [--validate N] [--load]
"""
...
import sqlglot
import templates as tpl  # the sibling module: scripts run with their own directory on sys.path
import yaml
...
LONG_TEXT = re.compile(
    r"(desc|description|subject|note|verbatim|narrative|body|url|user_agent|page_|properties|reason_codes)",
    re.I,
)
...
NONDET = re.compile(
    r"\b(CURRENT_DATE|CURRENT_TIMESTAMP|CURRENT_DATETIME|RAND|SESSION_USER|GENERATE_UUID|TABLESAMPLE)\b", re.I
)
...
class Sim:
    def __init__(
        self, cfg: dict, wh: dict, records: list[dict], estate: dict, days: int | None, scale: float
    ):
    ...
    def analyze(self, rec: dict) -> dict:
        ...
        info = {
            ...
            "nondet": False,
            "reads_bases": set(),
            "writes": set(rec.get("tables_written", [])),
        }
        ...
        for s in stmts:
            ...
            for c in s.find_all(exp.Column):
                ...
                if len(parts) >= 2 and parts[0] in alias:
                    ...
                    root = parts[0].lower()
                    for f in reads:
                        if used[f] is not None and any(
                            cc["name"].lower() == root for cc in self.T[f]["columns"]
                        ):
                            used[f].add(root)
        if not stmts:
                ...
                if f == b:
                    ...
                    base_cols = (
                        ...
                        if cols is None
                            ...
                            for c in self.T[f]["columns"]
                            if c["name"].lower() in cols
                            for leaf in c.get("lineage", [])
                            if leaf.rpartition(".")[0] == b
                        }
                    )
                prev = scan.get(b, set())
    ...
    def shards_available(self, t: dict, run: dt.date) -> list[str]:
        ...
        start, to = (
            str(sh["from"]),
            (run - dt.timedelta(days=1)).isoformat() if sh["to"] == "log_end" else str(sh["to"]),
        )
    ...
    def scan_bytes(self, info: dict, sql: str, run: dt.date) -> tuple[int, int, bool]:
        ...
        for b, cols in info["scan"].items():
            ...
            if t.get("shard"):
                ...
                m = re.search(
                    r"_TABLE_SUFFIX\s*(=|BETWEEN|>=)\s*'(\d{6,8})'(?:\s*AND\s*'(\d{6,8})')?", sql, re.I
                )
                if re.search(r"_TABLE_SUFFIX\s*=\s*FORMAT_DATE", sql, re.I) or m and m.group(1) == "=":
                    n = 1
            ...
            if t.get("partition"):
                ...
                for name in info["names"].get(b, ()):
                    m = re.search(
                        rf"\b{re.escape(name)}\b\)?\s*(>=|<=|=|>|<|BETWEEN)\s*(.{{0,120}})", sql, re.I | re.S
                    )
                    if not m:
                        continue
    ...
    def active(self, rec: dict, d: dt.date) -> bool:
        if rec.get("active_from") and d < dt.date.fromisoformat(rec["active_from"]):
            return False
        return not (rec.get("active_until") and d > dt.date.fromisoformat(rec["active_until"]))
    ...
    def principal_for(self, rec: dict, member: str | None = None) -> dict:
        ...
        m = member or (
            rec["members"][0]
            if rec["members"]
            else self.rng.choice(self.wh["teams"][rec["principal"]]["members"])
        )
        team = self.member_team[m]
        return {
            "email": f"{m}@{self.cfg['human_email_domain']}",
            "kind": "user",
            "member": m,
            "team": team,
            "tool": None,
    ...
    def gen_scheduled(self, events: list):
        ...
        dbt_models = {
            r["tables_written"][0]: r
            for rs in self.by_id.values()
            for r in rs
        ...
        for tid, recs in self.by_id.items():
            rec = recs[0]
            fam = rec["family"]
            if fam in ("human", "looker_query") or (
                fam == "tableau" and rec["schedule"].get("cadence") == "dashboard"
            ):
                continue
            ...
            for d in self.dates():
                ...
                if "once" in sched:
                    ...
                    fam == "dbt_build"
                    and sched.get("cron", "").endswith("2 * * *")
                    and sched["cron"].startswith("0 ")
                ):
                    ...
                    times.append(
                        self.local_dt(d, parse_hhmm(dbt_cfg["nightly_start"]), off + self.rng.uniform(0, 20))
                    )
                elif "cron" in sched:
    ...
    def gen_looker(self, events: list):
        ...
        for d in self.dates():
            ...
            for name, recs in tiles.items():
                for _ in range(
                    self.poisson(prng, lc["dashboard_views_per_day"].get(name, 5) * f * self.scale)
                ):
                    ...
                    for k, r in enumerate(recs):
                        events.append(
                            {
                                "t": t + dt.timedelta(milliseconds=150 * k),
                                "rec": r,
                                "why": "dashboard",
                                "who": self.principal_for(r),
                                "looker_user": uid,
                                "looker_member": m,
                            }
                        )
            for name, recs in explores.items():
                for _ in range(
                    self.poisson(prng, lc["explore_queries_per_day"].get(name, 2) * f * self.scale)
                ):
        ...
        for d in self.dates():
            ...
            for _ in range(self.poisson(prng, self.cfg["tableau"]["live_queries_per_day"] * self.scale)):
                ...
                events.append(
                    {
                        "t": self.local_dt(d, dt.time(8, 0), prng.uniform(0, 9 * 3600)),
                        "rec": r,
                        "why": "dashboard",
                        "who": self.principal_for(r),
                    }
    ...
    def execute(self, events):
        ...
        restricted_targets = [
            f
            for f, t in self.T.items()
            if self.wh["datasets"][t["dataset"]].get("restricted")
            and t["kind"] == "base"
        ...
        for e in events:
            ...
            if rec["job_type"] == "QUERY":
                ...
                if e.get("looker_user") is not None:
                    ctx = {
                        "user_id": e["looker_user"],
                        "history_slug": uuid.UUID(int=erng.getrandbits(128)).hex[:13],
                        "instance_slug": "7e3f0c2b9a4d4f1e8c6b5a0d3e2f1a9b",
                    }
                    sql = re.sub(
                        r"-- Looker Query Context '[^\n]*'",
                        "-- Looker Query Context '" + json.dumps(ctx, separators=(",", ":")) + "'",
                        sql,
                        count=1,
                    )
                    ...
                    if (
                        key in looker_cache
                        and (e["t"] - looker_cache[key]).total_seconds()
                        < self.cfg["looker"]["cache_minutes"] * 60
                    ):
                ...
                if who["kind"] == "user" and e["why"] == "adhoc" and erng.random() < hc["comment_noise"]:
                    note = erng.choice(
                        ["-- check", "-- for deck", "-- TODO clean up", "-- v2", "-- ask marcus", "-- temp"]
                    )
                    sql = f"{note}\n{sql}"
            ...
            if who["kind"] == "user" and e["why"] == "adhoc" and rec["job_type"] == "QUERY":
                if erng.random() < hc["errors"]["access_denied"]:
                    ...
                    denied = [
                        f
                        for f in restricted_targets
                        if not tpl.team_can_read(self.est, team, self.T[f]["dataset"])
                    ]
                    if denied:
                        ...
                        msg = (
                            f"Access Denied: Table {p}:{ds}.{tb}: User does not have permission to query table "
                            f"{p}:{ds}.{tb}, or perhaps it does not exist."
                        )
                        rows.append(
                            self.job_row(e, who, rec, bad, pn, error=("accessDenied", msg), attempted=f)
                        )
            ...
            nbytes, parts, pruned = (
                self.scan_bytes(info, sql, run_date) if rec["job_type"] == "QUERY" else (0, 0, True)
            )
            ...
            if rec.get("statement_type") == "SELECT" and not info["nondet"] and ckey in bq_cache:
                ...
                if (e["t"] - prev_end).total_seconds() < 86400 and all(
                    last_write.get(b, dt.datetime.min.replace(tzinfo=UTC)) < prev_end for b in bases
                ):
                    cache_hit = True
            error = None
            if (
            ...
            row = self.job_row(
                e, who, rec, sql, pn, nbytes=nbytes, parts=parts, info=info, cache_hit=cache_hit, error=error
            )
            rows.append(row)
    ...
    def typo(self, sql: str, rng) -> tuple[str | None, str | None]:
        cands = [
            m
            for m in re.finditer(r"\b([a-z][a-z0-9_]{5,})\b", sql)
            if m.group(1) not in KEYWORDS
            and "." not in sql[max(0, m.start() - 1) : m.start()]
            and sql[max(0, m.start() - 1) : m.start()] not in ("`", "'")
        ]
        if not cands:
            return None, None
        m = rng.choice(cands)
        ...
        return (
            sql[: m.start()] + bad + sql[m.end() :],
            f"Unrecognized name: {bad}; Did you mean {w}? at [{line}:{col}]",
        )

    def job_row(
        self, e, who, rec, sql, pn, nbytes=0, parts=0, info=None, cache_hit=False, error=None, attempted=None
    ) -> dict:
        ...
        if who["kind"] == "user" and who.get("tool") == "bigquery_console":
            ...
            job_id = (
                f"airflow_edw_nightly_balances_{rec['id'].split('.')[1]}_{created.strftime('%Y%m%dT%H%M%S')}"
        ...
        if error and error[0] in ("invalidQuery", "accessDenied"):
            ...
            slot = int(
                max(
                    12 + 5 * info["joins"],
                    nbytes / self.cfg["cost"]["bytes_per_slot_ms"] * cx * rng.lognormvariate(0, 0.35),
                )
            )
        ...
        billed = (
            0
            if not nbytes or error
            else max(self.cfg["cost"]["min_billed_bytes"], math.ceil(nbytes / 2**20) * 2**20)
        )
        # tables
        refs, dest = [], None
        if attempted:
        ...
        if rec.get("tables_written") and stmt != "SELECT":
            ...
            if rec["job_type"] == "LOAD" and rec.get("destination"):
                tb = re.sub(
                    r"\{\{\w+\}\}",
                    (created.astimezone(self.tz).date() - dt.timedelta(days=1)).strftime("%Y%m%d"),
            ...
            tbl = (
                "anon" + hashlib.sha256(sql.encode()).hexdigest()
                if info and not info["nondet"]
                else "anon" + str(uuid.UUID(int=rng.getrandbits(128))).replace("-", "_")
            )
            dest = {"project_id": project, "dataset_id": ads, "table_id": tbl}
        labels = []
        for k, v in (rec.get("labels") or {}).items():
            if "{{" in str(v):
                m = re.search(r"invocation_id\": \"([0-9a-f-]{36})", sql) or re.search(
        ...
        if stmt in ("MERGE", "INSERT", "DELETE") and not error:
            ...
            dml = {
                "inserted_row_count": n if stmt != "DELETE" else 0,
        ...
        return {
            ...
            "project_id": project,
            "project_number": pn[project],
            "user_email": who["email"],
            "principal_subject": f"{who['kind']}:{who['email']}",
            "job_id": job_id,
            ...
            "query": sql if is_query else None,
            "state": "DONE",
            "reservation_id": None,
            ...
            "error_result": (
                {"reason": error[0], "location": "query", "debug_info": None, "message": error[1]}
                if error
                else None
            ),
            ...
            "total_modified_partitions": (
                parts
                if stmt in ("MERGE", "INSERT", "DELETE", "CREATE_TABLE_AS_SELECT") and not error
                else None
            ),
            "query_info": (
                {"query_hashes": {"normalized_literals": normalized_hash(sql)}}
    ...
    def truth_row(self, row, e, rec, injected=None, attempted=None) -> dict:
        ...
        return {
            ...
            "member": who.get("member") or e.get("looker_member"),
            "team": who.get("team")
            or (self.member_team.get(e.get("looker_member")) if e.get("looker_member") else None),
            "tool": who.get("tool"),
            "why": e["why"],
            "session": e.get("session"),
            "looker_user_id": e.get("looker_user"),
...
def report(rows, truth, sim: Sim, validated: tuple[int, int] | None) -> str:
    L = [
        "# Simulated query log",
        "",
        f"Window {sim.start} to {sim.end} ({sim.days} days), seed {sim.cfg['seed']}, "
        f"scale {sim.scale}. {len(rows):,} jobs.",
        "",
    ]
    kinds = Counter(r["principal_subject"].split(":")[0] for r in rows)
    L += [
        f"- Service-account jobs: {kinds['serviceAccount'] / len(rows):.1%}; human jobs: {kinds['user'] / len(rows):.1%}"
    ]
    ...
    L += [
        ...
        "## People",
        "",
        f"{len(per_user)} people ran queries. Jobs per person: max {vals[0]:,}, "
        f"median {vals[len(vals) // 2]:,}, min {vals[-1]:,}.",
        "",
        "| Person | Jobs |",
        "|---|---|",
    ] + [f"| {u} | {n:,} |" for u, n in per_user.most_common(8)]
    looker_users = Counter(t["looker_user_id"] for t in truth if t["looker_user_id"] is not None)
    L += [
        "",
        f"Looker queries carry {len(looker_users)} distinct Looker user ids in their Query Context comment.",
    ]
    ...
    L += [
        "",
        "## Top 12 templates by bytes billed",
        "",
        "| Template | Jobs | TiB billed | Findings |",
        "|---|---|---|---|",
    ]
    ...
    if validated:
        L += [
            ...
            "## Validation",
            "",
            f"{validated[1]} distinct successful query texts sampled and dry-run in "
...
def finding_checks(rows, truth, sim: Sim):
    ...
    def reads(fqn, fam_exclude=("dq_monitor",)):
        return [
            r
            for r in rows
            if fqn in tmap[r["job_id"]]["tables_read"]
            and tmap[r["job_id"]]["family"] not in fam_exclude
    ...
    out.append(
        (
            "F11 churn v2 stragglers"
            + ("" if covers else " (window ends before the cutover - not applicable)"),
    ...
    out.append(
        (
            "F05 deprecated but load-bearing",
            len(writes(eom)) > 0
            and len(reads("fennmoor-analytics.sbx_finance.eom_bal_by_product_tableau")) > 0,
    ...
    out.append(
        (
            "F13 weekly CTAS of an always-empty join",
            len(writes(wtb)) >= sim.days // 7 - 1,
    ...
    first_close = min(
        (
            ...
            for r in rows
            if r["query"]
            and "wrapup_code_name = 'ACCT_CLOSE'" in r["query"]
            and r["principal_subject"].startswith("user:")
        ),
        default=None,
    )
    out.append(
        (
            "F17 new closure code appears in human filters on/after 2026-06-02",
            first_close is None
            and sim.end < dt.date(2026, 6, 2)
            or (first_close and first_close >= dt.date(2026, 6, 2)),
    ...
    cdr = [
        ...
        if r["query"]
        and "cdr_*" in r["query"]
        and "_TABLE_SUFFIX" not in r["query"]
        and not r["error_result"]
    ]
    out.append(
        (
            "F09 unfiltered shard scans",
            len(cdr) > 0,
    ...
    n, tib = bytes_of(
        lambda r, t: (
            bool(r["query"])
            and re.search(r"SELECT \*[^;]*\bLIMIT \d+\s*$", r["query"], re.I | re.S)
            and (r["total_bytes_billed"] or 0) > 2**30
        )
    )
    out.append(
        ("F20 SELECT * ... LIMIT still full-scans", n > 0, f"{n:,} jobs over 1 GiB, {tib:,.1f} TiB billed")
    )
    n, tib = bytes_of(lambda r, t: t["family"] in ("dbt_freshness", "looker_trigger"))
    out.append(
        ("F21 freshness and trigger checks that scan data", tib > 1, f"{n:,} jobs, {tib:,.1f} TiB billed")
    )
    n, tib = bytes_of(
        lambda r, t: t["template_id"] in ("dbt.build.fct_app_sessions", "dbt.build.int_digital_sessions")
    )
    out.append(
        ("F22 incremental models that rescan their source", tib > 1, f"{n:,} runs, {tib:,.1f} TiB billed")
    )
    return out
...
def main() -> int:
    ...
    ap.add_argument(
        "--load-only", action="store_true", help="load the existing build/log output, no re-simulation"
    )
    args = ap.parse_args()
    if args.load_only:
        load_to_bigquery(args.project, wh_cfg["gcloud_config"])
        return 0
    cfg = yaml.safe_load((SPEC / "simulation.yaml").read_text())
    wh = json.loads((BUILD / "warehouse.json").read_text())
    records = json.loads((BUILD / "templates.json").read_text())
    estate = yaml.safe_load((SPEC / "warehouse" / "00_estate.yaml").read_text())
