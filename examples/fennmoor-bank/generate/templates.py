...
Two sources:
  code   - derived deterministically from build/warehouse.json and
           spec/templates/bi.yaml: dbt builds/tests/freshness, ingestion
  ...
  human  - hand-written ad-hoc SQL in spec/templates/human/*.yaml, per team.
...
Usage: uv run examples/fennmoor-bank/generate/templates.py [--project P]   (default: estate.yaml warehouse.project)
...
def render_param(spec: str, rng: random.Random, run: dt.date) -> str:
    ...
    if kind == "looker_ctx":
        ctx = {
            "user_id": rng.randint(40, 400),
            "history_slug": "".join(rng.choice("0123456789abcdef") for _ in range(13)),
    ...
    for stmt in stmts:
        ...
        for tbl in stmt.find_all(exp.Table):
            if not tbl.db and tbl.name in ctes:
                continue
            if "INFORMATION_SCHEMA" in tbl.sql(dialect="bigquery").upper() or tbl.db.lower().startswith(
                "region-"
            ):
                continue
            if not tbl.db:
            ...
            if fqn is None:
                err(
                    f"{where}: references {'.'.join(p for p in (tbl.catalog, tbl.db, tbl.name) if p)}, not in the spec"
                )
                continue
...
def make_physical(est: Estate, project: str, prefix: str):
    ...
    def physical(sql: str) -> tuple[str, list[str]]:
        ...
        sql = full.sub(
            lambda m: (
                f"`{project}.{m[1]}{m[2]}`"
                if m[1].startswith("region-")
                else f"`{project}.{prefix}{m[1]}{m[2]}`"
            ),
            sql,
        )
        unq = []
...
def dbt_comment(name: str) -> str:
    return (
        '/* {"app": "dbt", "dbt_version": "1.9.4", "profile_name": "fennmoor", '
    ...
    for t in est.wh["tables"]:
        ...
        if mat == "view":
            ...
            if pks:
                ...
                sql = (
                    f"{head}merge into {target} as DBT_INTERNAL_DEST\n    using (\n{filt}\n    ) as DBT_INTERNAL_SOURCE\n"
                ...
                cond = (
                    (
                        f" and {pc.replace(pc.split('(')[-1].rstrip(')'), 'DBT_INTERNAL_DEST.' + pc.split('(')[-1].rstrip(')'))}"
                        f" >= date_sub(current_date(), interval 3 day)"
                    )
                    if pc
                    else ""
                )
    ...
    for t in est.wh["tables"]:
        ...
        if len(pks) == 1:
            ...
            for kind, inner in (
                ...
                sql = (
                    dbt_comment(f"{kind}_{t['name']}_{k}").replace("model.fennmoor", "test.fennmoor")
                    + DBT_TEST_HEAD
                    + inner
                    + "    ) dbt_internal_test"
                )
        ...
        for c in t["columns"]:
            ...
            sql = (
                dbt_comment(f"relationships_{t['name']}_{c['name']}").replace(
                    "model.fennmoor", "test.fennmoor"
                )
                + DBT_TEST_HEAD
                + inner
                + "    ) dbt_internal_test"
            ...
            out.append(
                Template(
                    f"dbt.test.relationships.{t['name']}.{c['name']}",
                    "dbt_test",
                    "dbt-prod",
                    "service_account",
                    "fennmoor-dw",
...
def gen_ingest(est: Estate, estate_yaml: dict) -> list[Template]:
    ...
    raw_specs = {}
    for f in sorted((SPEC / "warehouse").glob("1*.yaml")):
    ...
    for key, spec in raw_specs.items():
        ...
        if la and spec.get("stage") is not False and t.get("status") != "dead":
            sql = (
                f"select max({la}) as max_loaded_at,\n  current_timestamp() as snapshotted_at\n"
            ...
            out.append(
                Template(
                    ...
                    "service_account",
                    "fennmoor-dw",
                    '/* {"app": "dbt", "node_id": "source.fennmoor.' + key + '"} */\n' + sql,
                    schedule={"cron": "15 * * * *"},
                    source="warehouse.json",
                )
            )
        if "fivetran" in mx or "fivetran_salesforce" in mx:
            ...
            out.append(
                Template(
                    ...
                    sql,
                    params={"batch": "hex:12"},
                    schedule=refresh_schedule(refresh),
                    labels={"fivetran_connector": t["dataset"]},
                    validate_sub=[
                        (r"`fennmoor-raw\.fivetran_staging\.[^`]+`", f"(SELECT * FROM {fq(t)} WHERE FALSE)")
                    ],
                    source="warehouse.json",
                )
            )
...
def gen_legacy(est: Estate) -> list[Template]:
    out = []
    for key, date_col, cron, when in (
        ("legacy_edw.ACCT_DLY_BAL", "b.BAL_DT", "30 5 * * *", "DATE '{{ds}}'"),
        ...
        out.append(
            Template(
                f"legacy.delete.{t['name']}",
                "legacy_dag",
                "composer-legacy",
                "service_account",
                "fennmoor-analytics",
                f"DELETE FROM {fq(t)} WHERE {target_col} = {del_val}",
                params=params,
                schedule={"cron": cron},
                labels={"airflow_dag": "edw_nightly_balances"},
                source="warehouse.json",
            )
        )
        out.append(
            Template(
                f"legacy.insert.{t['name']}",
                "legacy_dag",
                "composer-legacy",
                "service_account",
                "fennmoor-analytics",
                f"INSERT INTO {fq(t)} ({cols})\n{t['sql']}{joiner}{date_col} = {when}",
                params=params,
                schedule={"cron": cron},
                labels={"airflow_dag": "edw_nightly_balances"},
                source="warehouse.json",
...
def gen_vertex(est: Estate, window: dict) -> list[Template]:
    ...
    out = [
        Template(
            "vertex.read.feat_customer_daily",
            "vertex",
            "vertex-scoring",
            "service_account",
            "fennmoor-dw",
    ...
    for name, frm, until in (
        ("churn_score_v2", None, before),
        ("churn_score_v3", cut.isoformat(), None),
...
def gen_sandbox_ctas(est: Estate, window: dict) -> list[Template]:
    ...
    for t in est.wh["tables"]:
        ...
        base = dict(
            family="sandbox_ctas",
            principal=est.member_team[writer],
            principal_kind="human",
            job_project="fennmoor-analytics",
            sql=sql,
            members=[writer],
            source="warehouse.json",
        )
        if in_window:
            out.append(
                Template(
                    f"ctas.once.{t['dataset']}.{t['name']}", **base, schedule={"once": created.isoformat()}
                )
            )
        if status == "active":
            out.append(
                Template(
                    f"ctas.{t['dataset']}.{t['name']}",
                    **base,
    ...
    for t in est.wh["tables"]:
        ...
        out.append(
            Template(
                ...
                "service_account",
                "fennmoor-analytics",
                f"{ctx}CREATE OR REPLACE TABLE {fq(t)} AS {t['sql']}",
                schedule={"cron": "0 6 * * *"},
                source="warehouse.json",
            )
        )
        ...
        date_col = None
        if src:
            date_col = part_col(src) or next(
                (c["name"] for c in src["columns"] if c.get("concept") == "date.day"), None
            )
        trig = f"SELECT MAX({date_col}) FROM {fq(src)}" if date_col else "SELECT CURRENT_DATE()"
        out.append(
            Template(
                f"looker.trigger.{t['name']}",
                "looker_trigger",
                "looker-prod",
                "service_account",
                "fennmoor-analytics",
...
def gen_hightouch(est: Estate) -> list[Template]:
    return [
        Template(
            ...
            f"SELECT * FROM {fq(t)}",
            schedule={"every": "1h"},
            labels={"hightouch_sync": t["name"]},
            source="warehouse.json",
        )
        for t in est.wh["tables"]
        if t["dataset"] == "reverse_etl" and t.get("status") != "dead"
    ]
    ...
    for t in est.wh["tables"]:
        ...
        out.append(
            Template(
                ...
                "service_account",
                "fennmoor-analytics",
                f"SELECT COUNT(*) AS row_count FROM {src}{filt}",
                schedule={"every": "4h"},
                labels=lbl,
                source="warehouse.json",
            )
        ...
        ts = (
            loaded_at_expr(raw_specs.get(key, {}), mixins)
        ...
        if ts:
            out.append(
                Template(
                    ...
                    "service_account",
                    "fennmoor-analytics",
                    f"SELECT MAX({ts}) AS max_ts FROM {src}",
                    schedule={"every": "4h"},
                    labels={"elementary_monitor": "freshness"},
                    source="warehouse.json",
                )
            )
        keys = [
            c["name"]
            for c in t["columns"]
            if c.get("flags", {}).get("pk") or c.get("flags", {}).get("required")
        ][:2]
        if keys:
            ...
            out.append(
                Template(
                    ...
                    "service_account",
                    "fennmoor-analytics",
                    f"SELECT {sel}, COUNT(*) AS total_rows FROM {src}{filt}",
                    schedule={"every": "4h"},
                    labels={"elementary_monitor": "null_rate"},
                    source="warehouse.json",
                )
            )
    for name, ts in (
        ...
        out.append(
            Template(
                ...
                members=["marcus.lindqvist"],
                notes="`edr report`, run each weekday morning",
                source="warehouse.json",
            )
        )
    out.append(
        Template(
            ...
            params={
                "rid": "uuid",
                "test": "choice:volume_anomalies|freshness_anomalies|null_rate",
...
LOOKER_DATE_FILTER = (
    "((( {f} ) >= ((DATE_ADD(CURRENT_DATE('America/Chicago'), INTERVAL -{n} DAY))) AND "
    ...
    for name, ex in (bi.get("explores") or {}).items():
        ...
        for i, tile in enumerate(ex.get("tiles", [])):
            sql = looker_sql(
                ...
                tile["measures"],
                tile.get("filters", []),
                tile.get("days", ex.get("default_days")),
                True,
                tile.get("limit", 500),
            )
            out.append(
                Template(
                    f"looker.tile.{name}.{i}",
                    "looker_query",
                    "looker-prod",
                    "service_account",
                    "fennmoor-analytics",
        ...
        for i in range(ex.get("explore_shapes", 0)):
            ...
            out.append(
                Template(
                    f"looker.explore.{name}.{i}",
                    "looker_query",
                    "looker-prod",
                    "service_account",
                    "fennmoor-analytics",
                    looker_sql(ex, name, d, m, f, days, rng.random() < 0.7, rng.choice([500, 500, 5000])),
                    params={"ctx": "looker_ctx", **fparams},
                    schedule={"weight": 1, "cadence": "adhoc"},
                    members=users,
                    findings=finds,
                    source="bi.yaml",
                )
            )
    for tb in bi.get("tableau") or []:
        sql = tb.get("sql")
        if tb.get("extract"):
...
class DryRunner:
    def __init__(self, project: str, config: str, enabled: bool):
        ...
        if enabled:
            ...
            self.bq = bigquery
            self.client = client(project, config, "US")
    ...
    def check(self, sql: str) -> dict:
        ...
        try:
            job = self.client.query(
                sql, job_config=self.bq.QueryJobConfig(dry_run=True, use_query_cache=False)
            )
...
def main() -> int:
    ...
    estate_yaml = yaml.safe_load((SPEC / "warehouse" / "00_estate.yaml").read_text())
    findings = yaml.safe_load((SPEC / "findings.yaml").read_text())["findings"]
    est = Estate(wh)
    ...
    templates = (
        gen_dbt(est)
        + gen_dbt_tests(est)
        + gen_ingest(est, estate_yaml)
        + gen_legacy(est)
        + gen_vertex(est, window)
        + gen_sandbox_ctas(est, window)
        + gen_looker_pdts(est)
        + gen_hightouch(est)
        + gen_dq(est, estate_yaml)
        + load_yaml_templates(est)
    )
            ...
            records.append(
                {
                    **asdict(t),
                    ...
                    "tables_written": [
                        est.logical(
                            *(
                                t.destination.split(".")[:1]
                                + t.destination.split(".")[1:2]
                                + [re.sub(r"\{\{\w+\}\}", "*", t.destination.split(".")[2])]
                            ),
                            t.job_project,
                        )
                        or t.destination
        ...
        for vi, shape in enumerate(expand_shapes(t)):
            ...
            for ds in unq:
                if est.project_of(ds) != t.job_project:
                    err(
                        f"{where}: unqualified {ds}.* resolves in {t.job_project}, but {ds} is in {est.project_of(ds)}"
                    )
            records.append(rec)
...
def write_report(records, est: Estate, findings):
    L = ["# Query templates", "", "Generated by `examples/fennmoor-bank/generate/templates.py`.", ""]
    ...
    for r in humans:
        ht[r["principal"]][0].add(r["id"])
        ht[r["principal"]][1] += 1
        ht[r["principal"]][2] |= set(r["tables_read"])
    ...
    unread = sorted(
        f
        for f, t in est.tables.items()
        if t.get("status", "active") == "active"
        and not (readers.get(f, set()) - {"dq_monitor"})
        and t["layer"] not in ("raw", "staging")
    )
    if unread:
        L += [
            "",
            "Active non-raw tables no template reads (besides the monitor): "
            + ", ".join(f"`{u}`" for u in unread),
        ]
    frozen_unread = sorted(
        f
        for f, t in est.tables.items()
        if t.get("status") == "frozen" and not (readers.get(f, set()) - {"dq_monitor"})
    )
    L += [
        "",
        f"Frozen tables nobody reads - dead in practice ({len(frozen_unread)}): "
    ...
    if counts:
        L += [
            ...
            + ", ".join(
                f"`{f}` ({shapes.get(f, 0)})"
                for f in sorted(modeled, key=lambda f: shapes.get(f, 0))
                if shapes.get(f, 0) < 3
            ),
        ]
    # findings
    ...
    emergent = {
        "F01": "every join on customer identifiers",
        "F02": "every join on customer identifiers",
        "F06": "absence of reads of status: dead tables",
        "F07": "writes, and absence of reads, of status: write_only tables",
        "F15": "every join on hub keys",
        "F19": "the dq_monitor family",
    }
    for fid in findings:
    ...
    L += ["", "## Statement types (from BigQuery dry runs)", ""] + [
    ...
    for fqn, t in est.tables.items():
        rd = sorted(
            {r["principal"] for r in records if fqn in r["tables_read"] and r["family"] != "dq_monitor"}
        )
        wr = sorted(
            {r["principal"] for r in records if fqn in r["tables_written"]}
            | (
                {
                    r["principal"]
                    for r in records
                    if r.get("job_type") == "LOAD" and r["tables_written"] and r["tables_written"][0] == fqn
                }
            )
        )
        usage[fqn] = {
