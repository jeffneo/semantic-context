"""Robustness of the semantic layer: noise injection and parameter sweeps.

Everything runs on qlsc's own functions (qlsc/cluster.py, qlsc/hierarchy.py) with the parameters in
src/qlsc/defaults.yaml as the baseline; nothing is written to the graph except temporary GDS projections. Agreement with the spec is NMI of the
...
subjects (level 1) or domains (level 2). Agreement between two runs is ARI over the units.
  ...
  2. Wrong joins: random pairs of identifier units merged into one, as a wrong join that got past
     the join-confidence check would do (qlsc/joins.py flags the kind production contradicts; a
     random one usually has no such evidence).
...
Writes results/robustness.md and .json.
Usage: uv run examples/fennmoor-bank/eval/robustness.py
"""
...
from common import Names, ari, build, nmi, settings, write_result

from qlsc import cluster as C
from qlsc import hierarchy as H
...
def main() -> int:
    ...
    for (
        r
    ) in G.rows("""MATCH (u)<-[:IS]-{0,1}(c:Column)<-[:HAS_COLUMN]-(t:Table) WHERE u:Variable OR u:Unjoined
                       RETURN u.id AS u, t.id AS t, count(c) AS n"""):
        unit_tables[r["u"]][r["t"]] += r["n"]
    ident = [
        r["u"]
        for r in G.rows(
            """MATCH (u) WHERE u:Variable OR (u:Unjoined AND u.name =~ $rx) RETURN u.id AS u""",
    ...
    def level1(shapes, flows, gamma=base_p["gamma"], seed=base_p["seed"], merge=None):
        ...
        C.project(G, C.edges_from(shapes, flows), base_p["lineage"])
    ...
    for rate in (0.05, 0.10, 0.25, 0.50, 1.00):
        for seed in (1, 2):
            ...
            res["noisy_queries"].append(
                {
                    "rate": rate,
                    "seed": seed,
                    "groups": n,
                    "ari_vs_clean": ari(of, base),
        ...
        for seed in (1, 2):
            ...
            while len(merge) < m:
                ...
                if (
                    a not in merge
                    and b not in merge
                    and a not in merge.values()
                    and table_of.get(a) != table_of.get(b)
                ):
                    merge[b] = a
            of, n = level1(shapes, flows, merge=merge)
            res["wrong_joins"].append(
                {
                    "merges": m,
                    "seed": seed,
                    "groups": n,
        ...
        res["gamma1"].append(
            {
                "gamma": g,
                "groups": runs[0][1],
                "nmi_subjects": nmi(tp, sub),
                "nmi_domains": nmi(tp, dom),
                "seed_ari": sum(ari(runs[i][0], runs[j][0]) for i, j in ((0, 1), (0, 2), (1, 2))) / 3,
            }
        )
    G.drop_projection(C.PROJECTION)
    # 4. level-1 -> level-2 sweep on the stored level-1 groups
    vecs = {
        r["id"]: r["e"] for r in G.rows("MATCH (s:Semantic {level: 1}) RETURN s.id AS id, s.embedding AS e")
    }
    ...
    for k in (3, 5, 8):
        ...
        for g2 in (1.5, 3.0, 4.5):
            ...
            tp = {
                ...
                if t in dom and c.most_common(1)[0][0] in parts[0]
            }
            res["level2"].append(
                {
                    "k": k,
                    "gamma": g2,
                    "groups": len(set(parts[0].values())),
                    "nmi_domains": nmi(tp, dom),
                    "seed_ari": sum(ari(parts[i], parts[j]) for i, j in ((0, 1), (0, 2), (1, 2))) / 3,
                }
            )
    G.drop_projection("robust")
    ...
    L = [
        ...
        "",
        f"Clean level 1: {n_base} groups, NMI vs spec subjects {base_nmi:.3f} (gamma {base_p['gamma']}). NMI = agreement of "
    ...
    for rate in sorted({r["rate"] for r in nq}):
        ...
        L.append(
            f"| {rate:.0%} ({int(rate * len(shapes))}) | {sum(r['groups'] for r in rs) / 2:.0f} | "
    ...
    for m in sorted({r["merges"] for r in wj}):
        ...
        L.append(
            f"| {m} | {sum(r['groups'] for r in rs) / 2:.0f} | {sum(r['ari_vs_clean'] for r in rs) / 2:.2f} | "
    ...
    L += [
        f"| {r['gamma']:g} | {r['groups']} | {r['nmi_subjects']:.3f} | {r['nmi_domains']:.3f} | {r['seed_ari']:.2f} |"
        for r in res["gamma1"]
    ]
    L += [
        "",
    ...
    L += [
        f"| {r['k']} | {r['gamma']:g} | {r['groups']} | {r['nmi_domains']:.3f} | {r['seed_ari']:.2f} |"
        for r in res["level2"]
    ]
    write_result("robustness", L, res)
    print("\n".join(L))
    G.close()
    return 0
