...
Graph: Neo4j `bigquery`. Keys: usage truth (successful statements in the window only).
...
Found pairs not in the truth: 33. Of these, 33 are invisible to the reference by construction (semi-joins, joins through CTEs/subqueries or transformed keys); 0 are not, and are listed for inspection:


Misses (in truth, not in the graph): 0


Found pairs invisible to the reference extractor (all, for checking by hand):
