...
# dependencies = ["google-cloud-bigquery>=3.25"]
# ///
"""Deploy the Fennmoor estate to BigQuery EMPTY, and validate every model.

Reads specs/build/warehouse.json (run tools/build.py first). Nothing is billed:
...
Usage:
  uv run specs/tools/deploy.py --project jeffdavis-bq-testproj [--config qlsc]
...
def main() -> int:
    ...
    client = bigquery.Client(project=args.project, credentials=GcloudCredentials(args.config), location="US")
    ...
    def deploy(t: dict) -> dict:
        r = {"fqn": t["fqn"], "kind": t["kind"], "materialized": t["materialized"],
             "status": "ok", "diffs": [], "error": None, "physical": []}
        name = t["name"]
        try:
            if t["kind"] == "base":
                names = ([f"{name}{s}" for s in shard_suffixes(t["shard"], wh["log_window"], args.all_shards)]
                         if t.get("shard") else [name])
                for n in names:
            ...
            ensure_type(f"{project}.{prefix}{t['dataset']}.{name}",
                        "VIEW" if t["materialized"] == "view" else "TABLE")
    ...
    print(f"\n{dict(counts)} - report: specs/build/deploy_report.md")
