...
Everything below leaves your default gcloud configuration and your
`neo4j-se-team-201905` default project untouched. Claude gets a **service
account it impersonates with your login** - no key file is ever created.
...
```bash
gcloud iam service-accounts create qlsc-bq --display-name="Query-log semantic context (Claude)" --project=$QLSC_PROJECT
```
...
```bash
gcloud iam service-accounts add-iam-policy-binding qlsc-bq@$QLSC_PROJECT.iam.gserviceaccount.com --member="user:jeff.davis@neo4j.com" --role="roles/iam.serviceAccountTokenCreator" --project=$QLSC_PROJECT
```
...
### 7. Create a separate gcloud configuration for Claude

A named configuration, not your default one. Claude's commands select it
explicitly with `CLOUDSDK_ACTIVE_CONFIG_NAME=qlsc`.
...
```bash
gcloud config set account jeff.davis@neo4j.com --configuration=qlsc
```
...
An empty listing (or your existing datasets, if shared) means it works. Then
tell Claude the project id.
...
trace. Claude won't delete anything without asking you first.
